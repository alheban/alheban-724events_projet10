...
const Select = ({
  ...
  return (
    <div className={`SelectContainer ${type}`} data-testid="select-testid">
      {label && <div id="label-type" className="label">{label}</div>}
      {error && <div className="error-select">{error}</div>}
      <div className="Select">
        ...
        <input type="hidden" value={value || ""} name={name} aria-labelledby={`${name}`} />
...
Select.propTypes = {
  ...
  error: PropTypes.string // Ajout de la prop error dans les propTypes
...
Select.defaultProps = {
  ...
  type: "normal",
  name: "select",
  error: "" // Définition de la valeur par défaut pour error
}
